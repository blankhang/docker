...
# 运行初始化脚本 即可自动完成 对应文件/目录创建 redis-stack.yml Dockerfile jenkins_home
sh init.sh

# 启动 jenkins
sh start.sh
# 或
docker-compose -f /docker/jenkins/redis-stack.yml up -d

# 重启 jenkins
docker-compose -f /docker/jenkins/redis-stack.yml restart

# 停止 jenkins
docker-compose -f /docker/jenkins/redis-stack.yml down
```
