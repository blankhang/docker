# Elasticsearch / Kibana 9.4.4（全插件版）

| 目录 | 用途 |
|------|------|
| `build/` | 自定义镜像（IK + 拼音 + 简繁） |
| `docker-compose/` | 单机 Compose 开发/测试 |
| `docker-stack/` | Swarm 多节点生产栈 |

镜像：`blankhang/elasticsearch:9.4.4`，相对 9.4.1/9.4.2（仅 IK）本版预装 INFINI 三件套（与 ES **9.4.4** 对齐，见 [release.infinilabs.com](https://release.infinilabs.com/)）：

| 插件 | 目录 | 作用 |
|------|------|------|
| **analysis-ik** | `plugins/ik` | 中文分词（`ik_max_word` / `ik_smart`） |
| **analysis-pinyin** | `plugins/analysis-pinyin` | 汉字 ↔ 拼音，支持全拼/首字母检索 |
| **analysis-stconvert** | `plugins/analysis-stconvert` | 简繁互转，统一字形后再检索 |
  ...
  "settings": {
    "analysis": {
      "filter": {
        ...
        "s2t_convert": { "type": "stconvert", "convert_type": "s2t", "keep_both": false },
        "t2s_convert": { "type": "stconvert", "convert_type": "t2s", "keep_both": false }
      },
      "analyzer": {
        "ik_smart_pinyin": {
          "tokenizer": "ik_smart",
...
- 正文检索：主字段用 IK 即可。
- 拼音搜中文：查 `title.pinyin`（或单独拼音字段）。
- 简繁：按业务在 index/search analyzer 上挂 `stconvert`（`s2t` / `t2s`，`keep_both` 控制是否保留原文）。

构建：

```bash
cd software/elasticsearch/9.4.4/build
docker build -t blankhang/elasticsearch:9.4.4 .
```

验证插件：

```bash
docker run --rm blankhang/elasticsearch:9.4.4 bin/elasticsearch-plugin list
# 应含 ik、analysis-pinyin、analysis-stconvert
```
...
多节点 Elasticsearch 集群 + 1 个 Kibana，镜像为 `blankhang/elasticsearch:9.4.4`（IK + 拼音 + 简繁）与官方 `kibana:9.4.4`。
...
- 已构建/拉取 `blankhang/elasticsearch:9.4.4`。
...
docker run --rm -it --user root -v "$PWD:/cwd" blankhang/elasticsearch:9.4.4 \
