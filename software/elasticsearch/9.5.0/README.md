...
官方发布说明：[Elasticsearch 9.5.0 release notes](https://www.elastic.co/docs/release-notes/elasticsearch#elasticsearch-9.5.0-release-notes)（2026-08-04 GA）。

## 9.5.0 更新概要（相对 9.4.x）

| 方向 | 内容 |
|------|------|
| 列式存储（Tech Preview） | 新 index mode：`columnar` / `logsdb_columnar`，降低磁盘占用 |
| 搜索性能 | 同节点多 shard 批量查询（`search.batched_query_phase`，默认开） |
| Reindex | 优雅停机可续、默认 PIT、专用管理 API |
| 数据生命周期 | DLM 支持 `frozen_after`（需 Enterprise） |
| ES\|QL / 时序 | flattened、date_range、TSDB temporality、ES95 codec 等 |
| 其它 | Lucene 10.5；审计日志可热开关等 |

**升级注意：**

- **IK 等插件**：需等 [INFINI](https://release.infinilabs.com/analysis-ik/stable/) 发布 `9.5.0` 包后再构建 `blankhang/elasticsearch:9.5.0`（当前最高到 9.4.4）。
- Kibana 使用同版本 `kibana:9.5.0`。
- `XPACK_*_ENCRYPTIONKEY` 须自行用 `openssl rand -base64 32` 生成（见下文 `.env`）。
...
多节点 Elasticsearch 集群 + 1 个 Kibana，镜像为 `blankhang/elasticsearch:9.5.0`（含 IK）与官方 `kibana:9.5.0`。

仓库默认 `es-stack.yml` 为 **3 节点**；可按需扩到 5 节点（见下文「无损扩容」）。

| 服务 | 节点约束（示例） | 发布端口 | 说明 |
|------|------------------|----------|------|
| es-node1 | `xhc-wh-1` | 19201→9200 | 与 Kibana 同机（可改） |
| es-node2 | `xhc-wh-2` | 19202→9200 | |
| es-node3 | `xhc-wh-3` | 19203→9200 | |
| es-node4 | `xhc-wh-4` | 19204→9200 | 扩容时新增 |
| es-node5 | `xhc-wh-5` | 19205→9200 | 扩容时新增 |
| kibana | `xhc-wh-1` | 15601→5601 | 连接各 `es-node*:9200` |
...
- 已构建/拉取 `blankhang/elasticsearch:9.5.0`。
...
docker run --rm -it --user root -v "$PWD:/cwd" blankhang/elasticsearch:9.5.0 \
